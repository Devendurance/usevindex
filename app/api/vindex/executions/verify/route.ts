...
// Accepts ONLY an execution ID. Read-only chain reads + PostgreSQL writes.
// Never calls KeeperHub and never writes onchain.
export async function POST(request: Request) {
  ...
  try {
    const result = await verifyEvacuationDestination({ env, db: getDb(), executionId: executionId.trim() });
