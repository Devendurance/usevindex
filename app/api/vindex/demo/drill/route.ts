...
import { runDemoDrillRoute } from "@/lib/vindex/demo-controller";
...
// Runs the REAL protection drill for the active demo run in the background.
// No body is accepted — the drill orchestrates the existing services with
// zero operator-controlled parameters.
export async function POST(request: Request) {
  ...
  try {
    const result = await runDemoDrillRoute(env, getDb());
